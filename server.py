...
def listar_usuarios():
    ...
    try:
        cur.execute("""
            SELECT id, nome, email, criado_em, ativo
            FROM usuarios
            ORDER BY id
        """)
        usuarios = cur.fetchall()
...
def editar_usuario(id):
    ...
    try:
        cur.execute("""
            ...
            WHERE id = %s
        """, (nome, email, generate_password_hash(senha), ativo, id))
        conn.commit()
        return jsonify({"ok": True})
    except Exception as e:
        conn.rollback()
...
def excluir_usuario(id):
    ...
    cur = conn.cursor()
    try:
        cur.execute("UPDATE usuarios SET ativo = FALSE WHERE id = %s AND ativo = TRUE", (id,))
        conn.commit()
        return jsonify({"ok": True})
    except Exception as e:
        conn.rollback()
